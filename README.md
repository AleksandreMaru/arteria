...
4. **Root directory:** leave **empty** (do NOT put `main` there — that is the branch name, not a folder)
5. **Node version:** `22.12.0` (Astro 6 requires Node `>=22.12.0`)
...
If the build log says `Cannot find cwd: .../repo/main`, open **Settings → Build configuration → Edit**, clear **Root directory**, save, and retry the deployment.
